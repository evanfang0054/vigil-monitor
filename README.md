...
这是一个前端项目稳定性监控系统，主要用于实时监控和收集线上项目的各类稳定性指标，包括性能指标、错误日志、用户行为和系统资源等核心监控维度。通过数据采集、清洗、分析和可视化，帮助开发团队快速发现和定位问题，提升项目的稳定性和用户体验。系统支持自动告警、性能分析、用户行为分析等多种数据分析功能，并提供完整的监控数据上报和处理流程。
